...
from eelbrain._experiment.preprocessing import (
    RawFilterElliptic,
    ica_input_name,
    raw_node_name,
)
...
def _test_result_manifest_path(
    ...
    *,
    ...
    return e._derivatives.resolve(
        node, state=e.state, options=options
    ).manifest_path
...
def test_sample(samples_experiment):
    ...
    wrapped_tree = e._show_dependencies(
        "evoked", max_line_length=60, return_str=True
    )
    ...
    with e._temporary_state:
        state = e.state
        state["subject"] = "*"
        assert str(ica_file_path(state, "*", datatype="meg")) == join(
            "derivatives", "mne", "sub-*", "meg", "sub-*_desc-*_ica.fif"
        )
        state["subject"] = "R0002"
        assert str(ica_file_path(state, "*", datatype="meg")) == join(
            "derivatives",
    ...
    with e._temporary_state:
        ...
        assert exists(
            e._derivatives.resolve(
                raw_node_name("1-40"), state=e.state
            ).manifest_path
        )
        ...
        assert exists(
            e._derivatives.resolve("cov", state=e.state).manifest_path
        )
    ...
    ds["mag"] = ds["mag"].sub(
        sensor=ds["mag"].sensor.index(exclude="MEG 0331")
    )  # load_evoked interpolates bad channel
    ...
    test_manifest = _test_result_manifest_path(
        e, "a>v", 0.05, 0.2, 0.05, samples=100, data="meg.rms", baseline=False
    )
    ...
    meg_rms = combine(meg.rms("sensor") for meg in megs).mean(
        "case", name="auditory"
    )
    ...
    meg_mean = combine(meg.mean("sensor") for meg in megs).mean(
        "case", name="auditory"
    )
    assert_dataobj_equal(res.c1_mean, meg_mean, decimal=21)
    res = e.load_test(
        "a>v", 0.05, 0.2, 0.05, samples=20, inv="", baseline=False
    )
    ...
    ep = e.load_epochs(
        baseline=True, epoch="visual", epoch_rejection=""
    ).aggregate("side")
    evs = e.load_evoked(
        baseline=True, epoch="visual-s", epoch_rejection="", model="side"
    )
    tstart = ep["mag"].time.tmin - shift
    assert_dataobj_equal(
        evs[0, "mag"], ep[0, "mag"].sub(time=(tstart, None)), decimal=19
    )
    tstop = ep["mag"].time.tstop + shift
    assert_almost_equal(
        evs[1, "mag"].x, ep[1, "mag"].sub(time=(None, tstop)).x, decimal=19
    )
    ...
    with pytest.raises(NotImplementedError):
        e.load_evoked(
            baseline=False, epoch="visual-s", epoch_rejection="", model="side"
        )
    ...
    v_target = e.load_epochs(baseline=True, epoch="visual")["mag"].sub(
        time=(-0.1, v.time.tstop)
    )
    assert_almost_equal(v.x, v_target.x)
    a = ds.sub("epoch=='auditory'", "mag").sub(time=(-0.1, 0.099))
    a_target = e.load_epochs(baseline=True, epoch="auditory")["mag"].sub(
        time=(0, 0.199)
    )
    ...
    e = Experiment(root)
    assert (
        e.get_field_values("subject", group="ab")
        == e.get_field_values("subject", group="alias")
        == ["R0000", "R0002"]
    )
    ...
    class Changed(SampleExperiment):
        variables = {
            "event": LabelVar(
                "value", {(1, 2, 3, 4): "target", 5: "smiley", 32: "button"}
            ),
            "side": LabelVar(
                "value", {(1, 3): "left", (2, 4): "right_changed"}
            ),
            "modality": LabelVar(
                "value", {(1, 2): "auditory", (3, 4): "visual"}
            ),
        }
        tests = {
    ...
    class Changed(Changed):
        variables = {
            ...
            "modality": LabelVar(
                "value", {(1, 2): "auditory", (3, 4): "visual_changed"}
            ),
        }
    ...
    class Experiment(SampleExperiment):
        raw = {
            **SampleExperiment.raw,
            "ica": RawICA(
                "1-40", "sample", method="fastica", n_components=0.95
            ),
    ...
    ica_manifest = e._derivatives.manifest_path(
        ica_path, ica_input_name("ica")
    )
    assert exists(ica_manifest)
    ica_manifest_data = json.loads(Path(ica_manifest).read_text())
    assert ica_manifest_data["resolve_state"] == {
        "subject": "R0000",
        "session": "",
        "acquisition": "",
    }
    ...
    assert not [
        entry
        for entry in e._derivatives.scan_cache().entries
        if entry.manifest_path == Path(ica_manifest)
    ]
    ...
    with pytest.raises(
        ...
        match="estimated using different settings for raw step",
    ) as error:
    ...
    assert not np.allclose(ds1["mag"].x, ds2["mag"].x, atol=1e-20), (
        "ICA change ignored"
    )
    # apply-ICA
    with catch_warnings():
...
def test_sample_source(samples_experiment):
    ...
    root = samples_experiment(
        n_subjects=3, n_segments=1, mris=True
    )  # TODO: use sample MRI which already has forward solution
    e = SampleExperiment(root)
    ...
    res_labels = e.load_test(
        "left=right", 0.05, 0.2, 0.05, samples=8, disconnect_labels=True
    )
    ...
    ds_left = e.load_evoked(
        model="side", cat=("left",), ndvar=False, inv="free-3-dSPM"
    )
    assert set(ds_all["side"].cells) == {"left", "right"}
    assert set(ds_left["side"].cells) == {"left"}
    assert ds_left.n_cases < ds_all.n_cases
    with open(
        _test_result_manifest_path(
            e, "left=right", 0.05, 0.2, 0.05, samples=8, data="source"
        )
    ) as fid:
    ...
    source_data_deps = source_manifest_data["dependencies"][
        "evoked-test-data"
    ]["dependencies"]
    assert source_data_deps["dataset"]["name"] == "evoked-stc-group-dataset"
    assert set(source_data_deps["dataset"]["dependencies"]) == {
        "R0000",
        "R0001",
        "R0002",
    }
    ...
    assert (
        disconnected_manifest_data["key"]["options"]["disconnect_labels"]
        is True
    )
    ...
    e.set(epoch="target")
    ress = e.load_test(
        "left=right", 0.05, 0.2, 0.05, samples=8, data="source.rms"
    )
    with open(
        _test_result_manifest_path(
            e, "left=right", 0.05, 0.2, 0.05, samples=8, data="source.rms"
        )
    ) as fid:
        roi_manifest_data = json.load(fid)
    assert "evoked-test-data" in roi_manifest_data["dependencies"]
    roi_deps = roi_manifest_data["dependencies"]["evoked-test-data"][
        "dependencies"
    ]
    ...
    assert all(
        group_deps[subject]["name"] == "evoked-stc" for subject in group_deps
    )
    assert all(
        "source-morph" not in group_deps[subject]["dependencies"]
        for subject in group_deps
    )
    ...
    ress = e.load_test(
        "twostage", 0.05, 0.2, 0.05, samples=8, data="source.rms"
    )
    with open(
        _test_result_manifest_path(
            e,
            ...
            0.2,
            0.05,
            node="two-stage-level-2",
            samples=8,
            data="source.rms",
        )
    ) as fid:
        two_stage_manifest_data = json.load(fid)
    assert "two-stage-level-1" in {
        dep["name"] for dep in two_stage_manifest_data["dependencies"].values()
    }
    ...
    assert (
        level_1_manifest_data["dependencies"]["two-stage-data"][
            "dependencies"
        ]["data"]["name"]
        == "evoked-stc"
    )
...
def test_sample_tasks(monkeypatch, samples_experiment):
    set_log_level("warning", "mne")
    from eelbrain._experiment.tests.sample_experiment_sessions import (
        SampleExperiment,
    )
    ...
    assert handle.artifact_path.is_relative_to(
        Path(root) / "derivatives" / "eelbrain" / "cache" / "raw@ica"
    )
    assert handle.artifact_path.suffix == ".fif"
    assert "_key-" in handle.artifact_path.name
    assert str(ica_file_path(e.state, "ica", datatype="meg")) == join(
        "derivatives", "mne", "sub-R0000", "meg", "sub-R0000_desc-ica_ica.fif"
    )
    ...
    bad_path = join(
        ...
        "sub-R0000",
        "meg",
        "sub-R0000_task-sample1_channels.tsv",
    )
    ...
    monkeypatch.setattr(
        recording_epochs_node, "build", count_recording_epochs_builds
    )
    ...
    target2_dependency = next(
        dep
        for dep in super_handle.node.dependencies(super_handle)
        if dep.label == "target2"
    )
    with e._temporary_state:
        e.set(epoch="target2")
        target2_entry = e._resolve_derivative(
            "epochs", options=target2_dependency.options
        ).describe_dependency()
    ...
    target = ds_super_keep.aggregate(
        "modality%side",
        drop=(
            "sample",
            "t_edf",
    ...
    rej_path = join(
        ...
        "sub-R0000",
        "meg",
        "sub-R0000_raw-1-40_epoch-target2_rej-manual_epoch.pickle",
    )
...
def test_ica_all_tasks_after_maxwell(samples_experiment):
    ...
    class Experiment(SampleExperiment):
        raw = {
            ...
            "ica": RawICA(
                "tsss", method="fastica", max_iter=1, n_components=0.95
            ),
            **SampleExperiment.raw,
        }
    ...
    assert str(
        ica_file_path(e.state, "ica", concatenate_runs=True, datatype="meg")
    ) == join(
        "derivatives", "mne", "sub-R0000", "meg", "sub-R0000_desc-ica_ica.fif"
    )
    with catch_warnings():
...
def test_epoch_reference(samples_experiment):
    ...
    for interpolate_bads in (False, True):
        ...
        ds_ref0 = e.load_epochs(
            reference="", interpolate_bads=interpolate_bads
        )
    ...
    assert not e_changed._resolve_derivative(
        "evoked", options={"model": "modality"}
    ).is_valid()
...
def test_interpolate_bads(samples_experiment):
    ...
    assert (
        bad not in e.load_epochs(interpolate_bads="keep")["mag"].sensor.names
    )
...
def test_interpolate_bads_after_ica(samples_experiment):
    ...
    e.make_bad_channels(
        bad
    )  # bad before fit -> excluded from the ICA decomposition
    with catch_warnings():
        filterwarnings("ignore", "FastICA did not converge", UserWarning)
        e.make_ica()
    ...
    assert (
        bad not in e.load_epochs(interpolate_bads="keep")["eeg"].sensor.names
    )
...
def test_channel_model_rejection(samples_experiment):
    ...
    assert (
        "cache" in cache_path.parts
        and "epoch-rejection-channel-model" in cache_path.parts
    )
    ...
    assert (
        n_rejected > 0
    )  # some epochs rejected (> max_interpolate bad channels)
    assert n_interp > 0  # some epochs have channels marked for interpolation
    assert (
        max(len(x) for x in rej_ds[INTERPOLATE_CHANNELS]) <= 2
    )  # never exceeds max_interpolate
    ...
    e_meg = Experiment(meg_root)
    e_meg.set(
        subject="R0000", epoch="target", raw="raw", epoch_rejection="auto"
    )
    with pytest.raises(ConfigurationError):
...
def test_channel_model_rejection_continuous(samples_experiment):
    ...
    assert rej_ds[
        "accept"
    ].x.all()  # windowed detection never rejects wholesale
...
def test_channel_model_rejection_variable_length(samples_experiment):
    ...
    assert all(
        isinstance(w, BadChannelWindow)
        for epoch_windows in windows
    ...
    ds0 = e.load_epochs(
        interpolate_bads=True, baseline=False, epoch_rejection=""
    )
    ds1 = e.load_epochs(
        interpolate_bads=True, baseline=False, epoch_rejection="auto"
    )
    assert isinstance(ds1["eeg"], Datalist)
    assert len(ds1["eeg"]) == len(windows)
    changed = zeroed_any = False
    for i, (y0, y1, epoch_windows) in enumerate(
        zip(ds0["eeg"], ds1["eeg"], windows)
    ):
        bad_by_channel = {}
        for w in epoch_windows:
        ...
        for ci, ch in enumerate(y0.sensor.names):
            ...
            if inside.any() and not np.array_equal(
                y0.x[ci, inside], y1.x[ci, inside]
            ):
                changed = True
    assert changed  # flagged windows were actually modified
    assert (
        zeroed_any
    )  # some interval had more than max_interpolate bad channels
...
def test_ransac_rejection(samples_experiment):
    ...
    assert (
        "cache" in cache_path.parts
        and "epoch-rejection-ransac" in cache_path.parts
    )
    assert rej_ds.n_cases == n_total
    n_rejected = int((~rej_ds["accept"]).sum())
    n_interp = sum(1 for x in rej_ds[INTERPOLATE_CHANNELS] if x)
...
def test_ransac_rejection_continuous(samples_experiment):
    ...
    class Experiment(SampleExperiment):
        epoch_rejection = {
            "auto": RANSACRejection(
                **RANSAC_REJECTION_KWARGS, continuous=0.1, window_len=0.1
            )
        }
...
def test_ransac_rejection_variable_length(samples_experiment):
    ...
    class Experiment(SampleExperiment):
        ...
        epoch_rejection = {
            "auto": RANSACRejection(**RANSAC_REJECTION_KWARGS, window_len=0.1)
        }
...
def test_evoked_backed_test_vars_are_post_aggregation_only(samples_experiment):
    ...
    class Experiment(SampleExperiment):
        tests = {
            **SampleExperiment.tests,
            "anova-ok": ANOVA(
                "modality * modality_num * subject",
                model="modality",
                vars={
                    "modality_num": LabelVar(
                        "modality", {"auditory": 0, "visual": 1}
                    )
                },
            ),
            "anova-bad": ANOVA(
                "modality_num * subject",
                vars={
                    "modality_num": LabelVar(
                        "modality", {"auditory": 0, "visual": 1}
                    )
    ...
    with pytest.raises(ConfigurationError, match="For evoked tests"):
        e._resolve_derivative(
            "evoked-test-data", options={**options, "test": "anova-bad"}
        ).load()
...
def test_raw_bad_channel_derivatives_follow_pipe_graph(samples_experiment):
    ...
    class Experiment(SampleExperiment):
        raw = {
            "ica": RawICA(
                "raw", ("sample1", "sample2"), "fastica", max_iter=1
            ),
    ...
    assert e.load_bad_channels(raw="ica", task="sample1") == [
        "MEG 0111",
        "MEG 0121",
    ]
    assert e.load_bad_channels(raw="ica", task="sample2") == [
        "MEG 0111",
        "MEG 0121",
    ]
    assert e.load_bad_channels(raw="apply-ica", task="sample1") == [
        "MEG 0111",
        "MEG 0121",
    ]
...
def test_raw_reader_warnings_are_summarized(monkeypatch, samples_experiment):
    ...
    with catch_warnings(record=True) as record:
        ...
        e.load_raw(raw="raw")
    assert not any(
        "issued during raw-input@raw" in str(w.message) for w in record
    )
    ...
    log_path = Path(
        next(
            ...
            for handler in e._log.handlers
...
def test_evoked_cached_load_bypasses_epochs(monkeypatch, samples_experiment):
    ...
    def fail(*args, **kwargs):
        raise AssertionError(
            "Evoked dataset load should not rebuild epochs on an evoked cache hit"
        )
...
@requires_mne_sample_data
def test_evoked_cached_load_applies_cat_without_rebuilding_epochs(
    monkeypatch, samples_experiment
):
    set_log_level("warning", "mne")
    from eelbrain._experiment.tests.sample_experiment_sessions import (
        SampleExperiment,
...
def test_evoked_cache_ignores_irrelevant_selected_events_changes(
    ...
    e.set(subject="R0000", epoch="target1", epoch_rejection="")
    assert not e._resolve_derivative(
        "evoked", options={"model": "modality"}
    ).is_valid()
    e.load_evoked(ndvar=False, model="modality")
    assert e._resolve_derivative(
        "evoked", options={"model": "modality"}
    ).is_valid()

    assert not e._resolve_derivative(
        "evoked", options={"model": "side"}
    ).is_valid()
...
def test_evoked_cache_stales_on_model_change(samples_experiment):
    ...
    class ChangedExperiment(SampleExperiment):
        variables = {
            **SampleExperiment.variables,
            "modality": LabelVar(
                "value", {(1, 2): "auditory_changed", (3, 4): "visual"}
            ),
        }

    e_changed = ChangedExperiment(root)
    e_changed.set(subject="R0000", epoch="target", epoch_rejection="")
    handle = e_changed._resolve_derivative(
        "evoked", options={"model": "modality"}
    )
...
def test_epochs_dependency_distinguishes_model_sensitivity(samples_experiment):
    ...
    e.set(subject="R0000", epoch="target", epoch_rejection="")
    evoked_handle = e._resolve_derivative(
        "evoked", options={"model": "modality"}
    )
    epochs_dep = next(
        dep
        for dep in evoked_handle.node.dependencies(evoked_handle)
        if dep.name == "epochs"
    )
    ...
    e_changed = ChangedExperiment(root)
    e_changed.set(subject="R0000", epoch="target", epoch_rejection="")
    evoked_handle_changed = e_changed._resolve_derivative(
        "evoked", options={"model": "modality"}
    )
    epochs_handle_changed = e_changed._resolve_derivative(
        "epochs", options=epochs_dep.options
    )
...
def test_recording_epochs_cache_uses_fif(samples_experiment):
    ...
    dep = next(
        dep
        for dep in epochs_handle.node.dependencies(epochs_handle)
        if dep.name == "recording-epochs"
    )
    handle = e._derivatives.resolve(
        dep.name, state={**e.state, **dep.state}, options=dep.options
    )
    epochs = handle.load()
    ...
    mtimes_1 = tuple(
        ...
        for path in sorted(handle.artifact_path.iterdir())
    )
...
def test_epochs_with_cached_recording_use_current_selected_events(
    ...
    dep = next(
        dep
        for dep in handle.node.dependencies(handle)
        if dep.name == "recording-epochs"
    )
    recording_handle = e._derivatives.resolve(
        dep.name, state={**e.state, **dep.state}, options=dep.options
    )
    ...
    mtimes_1 = tuple(
        path.stat().st_mtime_ns
        for path in sorted(recording_handle.artifact_path.iterdir())
    )
    ...
    dep_changed = next(
        dep
        for dep in handle_changed.node.dependencies(handle_changed)
        if dep.name == "recording-epochs"
    )
    recording_handle_changed = e_changed._derivatives.resolve(
        dep_changed.name,
        state={**e_changed.state, **dep_changed.state},
        options=dep_changed.options,
    )
    assert (
        recording_handle_changed.artifact_path
        == recording_handle.artifact_path
    )
...
def test_selected_events_manifest_uses_real_dependencies(samples_experiment):
    ...
    assert set(rec_events_deps["labeled-events"]["dependencies"]) == {
        "events-input",
        "events",
    }
...
def test_raw_cache_identity_ignores_view_options(samples_experiment):
    ...
    handle_default = e._resolve_derivative(
        node_name, options={"noise": False, "preload": False}
    )
    handle_view = e._resolve_derivative(
        node_name, options={"noise": False, "preload": True}
    )
    handle_noise = e._resolve_derivative(
        node_name, options={"noise": True, "preload": False}
    )
...
def test_raw_info_view_matches_source_and_processed_raws(samples_experiment):
    ...
    source_info = e._resolve_derivative(
        raw_node_name("raw"), options={"noise": False, "preload": False}
    ).load(view="info")
    ...
    processed_info = e._resolve_derivative(
        raw_node_name("1-40"), options={"noise": False, "preload": False}
    ).load(view="info")
    processed_raw = e.load_raw(raw="1-40")
    assert processed_info["bads"] == processed_raw.info["bads"] == ["MEG 0111"]
    assert processed_info["highpass"] == pytest.approx(
        processed_raw.info["highpass"]
    )
    assert processed_info["lowpass"] == pytest.approx(
        processed_raw.info["lowpass"]
    )
...
def test_raw_filter_elliptic_info_view_matches_artifact(samples_experiment):
    ...
    info = e._resolve_derivative(
        raw_node_name("ellip"), options={"noise": False, "preload": False}
    ).load(view="info")
...
def test_selected_events_vardef_is_local(samples_experiment):
    ...
    compact = Variables(
        {"grouped": LabelVar("value", {(1, 2): "target"}, task="sample")}
    )
    changed = Variables(
        {
            "grouped": LabelVar(
                "value", {1: "target", 2: "nontarget"}, task="sample"
            )
        }
    )
    ...
    with pytest.raises(TypeError, match="undeclared option"):
        e._resolve_derivative(
            "epoch-events", options={**options, "vardef": compact}
        )
...
def test_primary_epoch_run(samples_experiment):
    ...
    class MultiRunExperiment(Pipeline):
        ...
        epochs = {
            "target": PrimaryEpoch(
                "sample", "event == 'target'", tmax=0.3, decim=5
            ),
            "target-copy": SecondaryEpoch("target"),
            "target-r1": PrimaryEpoch(
                "sample", "event == 'target'", tmax=0.3, decim=5, run="1"
            ),
            "target-r2": PrimaryEpoch(
                "sample", "event == 'target'", tmax=0.3, decim=5, run="2"
            ),
        }
    ...
    assert e.get("run") == "1", (
        "explicit-run epoch should set run='1' in state"
    )
    ...
    assert ds_all.n_cases == ds_r1.n_cases + ds_r2.n_cases, (
        f"combine-all ({ds_all.n_cases}) != run-1 ({ds_r1.n_cases}) + run-2 ({ds_r2.n_cases})"
    )
...
def test_load_trf(samples_experiment):
    ...
    e = SampleTRF(root)
    e.set(
        subject="R0000", epoch="target", epoch_rejection="", raw="1-40", inv=""
    )
    ...
    # cache hit
    options = e._trf_options(
        "imp", 0.0, 0.1, "boosting", None, None, None, False, {}
    )
...
def test_predictor_subset_fingerprint(samples_experiment):
    ...
    def write(stim, value, unused):
        ...
        ds = Dataset(
            {
                "time": Var([0.0, 0.1, 0.2, 0.3, 0.4]),
                "value": Var(value),
                "mask": Var(np.array([True, True, True, True, False])),
                "unused": Var(unused),
            }
        )
        ...
        save.pickle(ds, path)
        mtime[0] += (
            1  # ensure the quick (mtime) fingerprint changes between writes
        )
        os.utime(path, (mtime[0], mtime[0]))

    def read_reference(stim):
        return json.loads(
            (ref_dir / f"{stim}~word-value-mask.json").read_text()
        )
    ...
    assert (
        reference["source"]["mtime"] == mtime[0]
    )  # refreshed for the fast path
    ...
    assert read_reference("auditory")["version"] == {
        "uid": version_0["uid"],
        "serial": 1,
    }
    ...
    write(
        "auditory", [2.0, 2.0, 2.0, 2.0, 2.0], [9.0, 9.0, 9.0, 9.0, 9.0]
    )  # touch to force a quick-fingerprint mismatch
    assert not e._resolve_derivative("trf", options=options).is_valid()
...
def test_load_trf_filepredictor(samples_experiment):
    ...
    predictor_ndvars = {
        stim: NDVar(rng.normal(size=60), uts, name="env")
        for stim in ("auditory", "visual")
    }
    ...
    options = e._trf_options(
        "env", 0.0, 0.1, "boosting", None, None, samplingrate, False, {}
    )
    ctx = e._resolve_derivative("trf", options=options)
    assert ctx.is_valid()
    ...
    save.pickle(
        NDVar(rng.normal(size=60), uts, name="env"),
        pdir / "auditory~env.pickle",
    )
    assert not e._resolve_derivative("trf", options=options).is_valid()
...
def test_load_trfs_collection(samples_experiment):
    ...
    from eelbrain._experiment.tests.sample_experiment import (
        SampleExperiment,
        SampleTRF,
    )
    ...
    class SampleTRFCollection(SampleTRF):
        epochs = {
            **SampleExperiment.epochs,
            "avc": EpochCollection(("auditory", "visual")),
        }

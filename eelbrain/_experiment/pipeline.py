...
from .epoch_rejection import (
    ...
    ManualRejection,
    RANSACRejection,
    RANSACRejectionDerivative,
    RejectionInput,
)
from .events import (
    EpochEventsDerivative,
    EventsDerivative,
    EventsInput,
    LabeledEventsDerivative,
    SelectedEventsDerivative,
)
...
from .trf import (
    ...
    NUTSPredictor,
    PredictorInput,
    TRFDatasetDerivative,
    TRFDerivative,
    TRFGroupDatasetDerivative,
    TRFJob,
    TRFJobSpec,
    UTSPredictor,
    filter_predictor,
...
class Pipeline(StateModel):
    ...
    _default_parcs = {
        ...
        "lobes-op": CombinationParc(
            "lobes",
            {"occipitoparietal": "occipital + parietal"},
            ("lateral", "medial"),
        ),
        "lobes-ot": CombinationParc(
            "lobes",
            {"occipitotemporal": "occipital + temporal"},
            ("lateral", "medial"),
        ),
    }
    ...
    def __init__(
        ...
        **state,
        ...
        self._subjects = tuple(
            get_entity_vals(root, "subject", **ignore_entities)
        )
        self._sessions = tuple(
            get_entity_vals(root, "session", **ignore_entities)
        )
        ...
        if self.datatype is not None:
            if self.datatype not in ("meg", "eeg"):
                raise ConfigurationError(
                    f"`datatype` must be 'meg' or 'eeg', not {self.datatype!r}."
                )
            if not isinstance(self.extension, str):
                raise TypeError(
                    f"{self.__class__.__name__}.extension={self.extension!r} with {self.__class__.__name__}.datatype={self.datatype!r}; extension needs to be specified (e.g., '.fif')."
                )
            ...
            if "meg" in datatypes and "eeg" in datatypes:
                raise ConfigurationError(
                    f"Can't infer datatype. Both MEG and EEG data found in {root}."
                )
                ...
                data_extensions = {
                    path.extension
                    for path in mne_bids.find_matching_paths(
                        root,
                        datatypes="eeg",
                        suffixes="eeg",
                        extensions=[".edf", ".vhdr", ".set", ".bdf", ".fif"],
                    )
                }
                if len(data_extensions) == 0:
                    raise FileMissingError(
                        f"No EEG data files found in {root}."
                    )
                elif len(data_extensions) > 1:
                    raise ConfigurationError(
                        f"Multiple EEG data file types found in {root}: {enumeration(sorted(data_extensions))}."
                    )
                extensions = tuple(data_extensions)
            else:
                raise ConfigurationError(
                    f"Can't infer datatype. No MEG or EEG data found in {root}."
                )
        self._raw_extension = extensions[0]
        self._datatype = datatype

        acquisitions = tuple(
            get_entity_vals(root, "acquisition", **ignore_entities)
        )
        ...
        self._acquisitions = tuple(
            sorted({path.acquisition or "" for path in matching_paths})
        )
        ...
        self._recordings: frozenset[tuple[str, str, str, str, str]] = (
            frozenset(
                (
                    path.subject or "",
                    path.session or "",
        ...
        self._runs_for: dict[tuple[str, str, str, str], list[str]] = {
            key: sorted(runs) for key, runs in runs_seen.items()
        }
        ...
        self._log = log = logging.Logger(
            self.__class__.__name__, logging.DEBUG
        )
        initialized = datetime.now()
        log_file = _session_log_file(
            root / LOG_DIR, self.__class__.__name__, initialized
        )
        os.makedirs(log_file.parent, exist_ok=True)
        handler = logging.FileHandler(log_file)
        formatter = StructuredFormatter(
            "%(levelname)-8s %(asctime)s %(message)s", "%m-%d %H:%M"
        )
        ...
        self._screen_log_level = log_level(
            screen_log_level or self.screen_log_level
        )
        ...
        ######################
        # groups
        self._groups = ConfigurationDict(
            "group", assemble_groups(self.groups, set(self._subjects))
        )

        # preprocessing
        self._raw = assemble_raw_pipes(
            {"raw": RawSource(), **self.raw}, self._tasks
        )
        ...
        # epochs
        self._epochs = ConfigurationDict(
            "epoch", assemble_epochs(self.epochs, self._tasks)
        )
        ...
        for name, rejection in self.epoch_rejection.items():
            if not isinstance(name, str):
                raise TypeError(
                    f"epoch_rejection[{name!r}]: name must be a string"
                )
            elif not name:
                raise ValueError(
                    f"epoch_rejection[{name!r}]: name can't be empty"
                )
            elif not isinstance(rejection, EpochRejection):
                raise TypeError(
                    f"epoch_rejection[{name!r}]={rejection!r}: need EpochRejection"
                )
            epoch_rejection[name] = rejection
        self._epoch_rejection = ConfigurationDict(
        ...
        for name, reference in self.references.items():
            ...
            elif not isinstance(reference, Reference) or isinstance(
                reference, RawPipe
            ):
                raise TypeError(
                    f"references[{name!r}]={reference!r}: need Reference"
                )
            elif name == "average":
                if reference.reference != "average":
                    raise ConfigurationError(
                        f"references[{name!r}]={reference!r}: the standard average reference must be an average reference"
                    )
                elif reference.drop:
                    raise ConfigurationError(
                        f"references[{name!r}]={reference!r}: the standard average reference can not drop channels"
                    )
        ...
        for name, cov in self._covs.items():
            if not isinstance(cov, (RawCovariance, EpochCovariance)):
                raise TypeError(
                    f"_covs[{name!r}]={cov!r}: need RawCovariance or EpochCovariance"
                )
        ...
        self._parcs = ConfigurationDict(
            "parcellation", {**self._default_parcs, **self.parcs}
        )
        ...
        for name, estimator in estimators.items():
            if not isinstance(estimator, Estimator):
                raise TypeError(
                    f"estimators[{name!r}]={estimator!r}: need Estimator"
                )
        ...
        if not isinstance(self.stim_var, str):
            raise TypeError(
                f"{self.__class__.__name__}.stim_var={self.stim_var!r}"
            )
        ...
        self._register_field(
            ...
            self._tasks,
            depends_on=("epoch",),
            slave_handler=self._update_task,
            repr=True,
        )
        self._register_field(
            "acquisition",
            ...
            repr=True,
            allow_empty=True,
            depends_on=("epoch", "subject", "session", "task"),
            slave_handler=self._update_acquisition,
        )
        self._register_field(
            "run",
            self._runs,
            repr=True,
            depends_on=("epoch", "subject", "session", "task", "acquisition"),
            slave_handler=self._update_run,
        )
        self._register_field(
            "equalize_evoked_count", ("", "eq"), allow_empty=True
        )
        self._register_field("common_brain", ("fsaverage",))

        self._register_field(
            "mri", sorted(self._mri_subjects), allow_empty=True
        )
        self._register_field(
            "group",
            self._groups.keys(),
        ...
        self._register_field(
            "raw", sorted(self._raw), default=raw_default, repr=True
        )
        self._register_field(
            "epoch_rejection", self._epoch_rejection.keys(), allow_empty=True
        )
        self._register_field(
            "reference", self._references.keys(), allow_empty=True
        )
        ...
        self._register_field(
            "inv", default="", eval_handler=self._eval_inv, allow_empty=True
        )
        ...
        if self.default_data is None:
            ...
            if not data.sensor or data.string.split(".", 1)[0] == "sensor":
                raise ConfigurationError(
                    f"{self.__class__.__name__}.default_data={self.default_data!r}; must be a specific sensor type ('mag', 'grad', or 'eeg')."
                )
        ...
        self._register_field(
            "adjacency", ("", "link-midline"), allow_empty=True
        )
        ...
        log.info(
            "*** %s initialized with root %s on %s ***",
            self.__class__.__name__,
            root,
            initialized.strftime("%Y-%m-%d %H:%M:%S"),
        )
        level = (
            logging.DEBUG
        ...
        log.log(
            level, "Using eelbrain %s, mne %s.", __version__, mne.__version__
        )
        # Legend for the tab-separated columns appended to cache-event log lines (DEBUG, file only).
        log.debug(
            "Cache-event columns (tab-separated after the message): %s",
            "\t".join(CACHE_EVENT_COLUMNS),
        )
    ...
    def _init_derivative_registry(self):
        self._derivatives = DerivativeRegistry(
            self.root, self._log, self._datatype
        )
        result_args = (
            self.tests,
            self._epochs,
        ...
        for raw_name, pipe in self._raw.items():
            if isinstance(pipe, RawSource):
                ...
                self._derivatives.register(
                    RawBadChannelsInput(raw_name, pipe, self._raw_extension)
                )
                self._derivatives.register(
                    RawSourceDerivative(raw_name, pipe, self._raw_extension)
                )
                self._derivatives.register(
                    RawHeadPositionDerivative(raw_input.name)
                )
                self._derivatives.register(
                    CanonicalHeadPositionDerivative(
                        self._recordings, self._tasks, self._runs
                    )
                )
            elif isinstance(pipe, CachedRawPipe):
                self._derivatives.register(
                    RawDerivative(
                        raw_name, pipe, self._raw, self._raw_extension
                    )
                )
                if isinstance(pipe, RawICA):
                    self._derivatives.register(
                        ICAInput(raw_name, pipe, self._recordings, self._runs)
                    )
        ...
        self._derivatives.register(
            RejectionInput(self.root, self._epoch_rejection, self._epochs)
        )
        self._derivatives.register(
            ChannelModelRejectionDerivative(
                self._epochs, self._epoch_rejection
            )
        )
        self._derivatives.register(
        ...
        self._derivatives.register(
            TRFDatasetDerivative(self.root, self._estimators, self._epochs)
        )
        self._derivatives.register(
            TRFGroupDatasetDerivative(self._mri_subjects, self._groups)
        )
        ...
        self._derivatives.register(
            EventsDerivative(
                self.trigger_shift,
                sequence_arg(
                    f"{self.__class__.__name__}.stim_channel",
                    self.stim_channel,
                ),
                self.merge_triggers,
                self.preload,
        ...
        self._derivatives.register(
            SelectedEventsDerivative(self._epochs, self._epoch_rejection)
        )
        self._derivatives.register(
            EpochEventsDerivative(self._epochs, self._runs_for)
        )
        self._derivatives.register(
            RecordingEpochsDerivative(
                self._raw, self._epochs, self._references, self.cache_epochs
            )
        )
        self._derivatives.register(
            EpochsDerivative(self._raw, self._epochs, self._runs_for)
        )
        self._derivatives.register(EvokedDerivative(self._raw, self._epochs))
        self._derivatives.register(
            EvokedGroupDatasetDerivative(self._raw, self._groups)
        )

        # --- Source-space infrastructure ---
        self._derivatives.register(
            CovDerivative(
                self._covs, self._raw, self._references, self._recordings
            )
        )
        self._derivatives.register(SrcDerivative())
        self._derivatives.register(SourceMorphDerivative())
        self._derivatives.register(
            FwdDerivative(self._raw, self._references, self._recordings)
        )
        self._derivatives.register(
            InvDerivative(
                self._raw, self._references, self._recordings, self.cache_inv
            )
        )
        ...
        self._derivatives.register(
            EpochsStcDerivative(self._raw, self._epochs, self._references)
        )
        self._derivatives.register(
            EvokedStcDerivative(self._raw, self._epochs, self._references)
        )
        self._derivatives.register(
            EvokedStcGroupDatasetDerivative(self._mri_subjects, self._groups)
        )

        # --- Statistical tests ---
        self._derivatives.register(
            EvokedTestDataDerivative(self.tests, self._epochs, self._groups)
        )
        self._derivatives.register(
            TwoStageDataDerivative(self.tests, self._epochs, self._groups)
        )
    ...
    def _resolve_derivative(
        ...
        return self._derivatives.resolve(
            name, state=self.state, options=options, controls=controls
        )
    ...
    def _load_derivative(
        ...
        *,
        ...
        ctx = self._resolve_derivative(
            name, options=options, controls=controls
        )
    ...
    def clean_cache(
        ...
        while not delete:
            command = ask(
                ...
                {
                    "delete": "permanently delete the listed files",
                    "list": "List files that will be deleted",
                    "abort": "keep everything",
                },
            ...
            if subjects in self.get_field_values("group"):
                if "group" in kwargs:
                    if kwargs["group"] != subjects:
                        raise ValueError(
                            f"group={kwargs['group']!r} inconsistent with subject={subjects!r}"
                        )
                    self.set(**kwargs)
                else:
    ...
    def get_field_values(
        ...
        **state,
        ...
        if field == "mrisubject":
            ...
            mrisubjects = [
                "sub-" + s
                for s in mrisubjects
                if (s != common_brain and not s.startswith("sub-"))
            ]
            return mrisubjects
        else:
    ...
    def iter(
        ...
        **state,
        ...
        ...
            State parameters.
        """
        return StateModel.iter(
            self, fields, exclude, values, progress_bar, **state
        )
    ...
    def load_bad_channels(self, noise: bool = False, **kwargs):
        ...
        return self._load_derivative(
            raw_node_name(raw_name), options={"noise": noise}, view="bads"
        )
    ...
    def _resolve_data(
        ...
        if source_space and not spec.source:
            raise ValueError(
                f"data={data!r} is sensor-space data, but the analysis is in source space (inv={self.get('inv')!r}); set inv='' for sensor-space analysis"
            )
        if not source_space and spec.source:
            raise ValueError(
                f"data={data!r} is source-space data, but the analysis is in sensor space (inv=''); set a non-empty inverse for source-space analysis"
            )
        if spec.sensor and spec.string.split(".", 1)[0] == "sensor":
            raise ValueError(
                f"data={data!r}: 'sensor' selects all sensor types; specify a single type ('mag', 'grad', 'eeg') for analysis"
            )
        return spec
        ...
        **state,
        ...
        if self.get("inv", **state):  # source space
            if (
                ...
                raise ValueError(
                    "interpolate_bads/tmin/tmax/tstop are not available for source-space epochs; set inv='' for sensor space"
                )
            if isinstance(ndvar, str):
                raise ValueError(
                    f"{ndvar=}: a data-kind ndvar is only valid for sensor-space epochs; in source space use ndvar=True or ndvar=False"
                )
        ...
        **state,
        ...
        if not isinstance(predictor, (UTSPredictor, NUTSPredictor)):
            raise NotImplementedError(
                f"{term.string}: load_predictor only supports file predictors; load {type(predictor).__name__} through load_trf"
            )
    ...
    def _trf_options(
        ...
        if mask is not None:
            raise NotImplementedError(
                f"{mask=}: source-space masking is not implemented yet"
            )
        ...
        if est.requires_sensor_space:
            if inv := self.get("inv"):
                raise ValueError(
                    f"{inv=} for {estimator=}: {estimator} uses sensor data and localizes internally; set inv='' for sensor-space analysis"
                )
            elif data is not None:
                raise ValueError(
                    f"{data=}: estimator {estimator!r} uses all sensor data; leave data unset"
                )
            data_string = "sensor"
        else:
    ...
    def load_trfs(
        ...
        **state,
        ...
        trf_options = self._trf_options(
            x, tstart, tstop, estimator, data, mask, samplingrate, filter_x
        )
        options = {
            **trf_options,
            "scale": scale,
    ...
    def load_evoked(
        ...
        **state,
        ...
        if inv := self.get("inv"):  # source space
            if interpolate_bads:
                raise ValueError(
                    "interpolate_bads not available for source-space data; set inv='' for sensor space"
                )
            if isinstance(ndvar, str):
                raise ValueError(
                    f"{ndvar=} with {inv=}: a data-kind ndvar is only valid for sensor-space evoked; in source space use ndvar=True or ndvar=False"
                )
            self._current_source_parc()
            options = {
                "model": model,
            ...
            if group is not None:
                return self._load_derivative(
                    "evoked-stc-group-dataset", options=options
                )
        ...
        if group is not None:
            ...
            return self._load_derivative(
                "evoked-group-dataset", options=options
            )
    ...
    def load_fwd(
        ...
        **state,
        ...
        if ndvar:
            ...
            if parc:
                self.make_annot()
            fwd = load.mne.forward_operator(
                fwd, src, self.root / MRI_SDIR, parc, adjacency=False
            )
            if parc:
    ...
    def load_inv(
        ...
        **state,
        ...
        if ndvar:
            parc = self._current_source_parc()
            inv = load.mne.inverse_operator(
                inv, self.get("src"), self.root / MRI_SDIR, parc
            )
            if parc:
    ...
    def load_label(
        ...
        **kwargs,
        ...
        if label in labels:
            ...
            raise ValueError(
                f"Label {label!r} could not be found in parc {self.get('parc')!r}."
            )
        ...
        **state,
        ...
        if group is not None:
            ...
            if state:
                self.set(**state)
            lines = [
                (subject, self.load_neighbor_correlation(1, epoch))
                for subject in self
            ]
    ...
    def load_raw(
        ...
        **kwargs,
        ...
        if decim and decim > 1:
            assert samplingrate is None, (
                "samplingrate and decim can't both be specified"
            )
        ...
        if ndvar:
            ...
            sysname = source_pipe._get_sysname(
                raw.info, self.get("subject"), data_kind
            )
        ...
        if group is not None:
            return combine(
                [
                    ...
                    for subject_ in self.iter(group=group)
                ]
            )
    ...
    def load_test(
        ...
        **state,
        ...
        NDTest
        | ROITestResult
        | tuple[Dataset | ROIData, NDTest | ROITestResult]
    ):
    ...
    def make_bad_channels(
        ...
        **kwargs: Any,
        ...
        if isinstance(bad_chs, (str, int)):
            bad_chs = (bad_chs,)
        raw = self._load_derivative(
            raw_input_name(source_name), options={"noise": noise}
        )
        bads_ctx = self._resolve_derivative(
            raw_bad_channels_input_name(source_name), options={"noise": noise}
        )
    ...
    def make_bad_channels_auto(
        ...
        **state: Any,
        ...
        raw_ctx = self._resolve_derivative(
            raw_input_name(source_name),
            options={"noise": noise, "preload": True},
        )
        raw = raw_ctx.load()
        bads_ctx = self._resolve_derivative(
            raw_bad_channels_input_name(source_name), options={"noise": noise}
    ...
    def make_bad_channels_neighbor_correlation(
        ...
        **state,
        ...
        .. warning::
            ...
            same value.
        """
        data, full_nc = self.load_neighbor_correlation(
            1, epoch, return_data=True, **state
        )
        bad_chs = []
        nc = full_nc
        while nc.min() < r:
    ...
    def make_ica_selection(
        ...
        **state,
        ...
        bads = self._load_derivative(
            raw_node_name(ica_name), options={"noise": False}, view="bads"
        )
        labeled_events = None
        if epoch is None:
            if task is None:
            ...
            decim = decim_param(
                samplingrate, decim, None, raw.info, minimal=True
            )
            info = raw.info
            display_data = raw
            ...
            if task:
                ...
                with self._temporary_state:
                    for state in ctx.node._source_states(ctx, task):
                        ...
                        offset += (
                            ds_t.info["raw.last_samp"]
                            - ds_t.info["raw.first_samp"]
                            + 1
                        ) / ds_t.info["raw.samplingrate"]
            ...
            if isinstance(ds["epochs"], Datalist):  # variable-length epoch
                data = np.concatenate(
                    [epoch.get_data()[0] for epoch in ds["epochs"]], axis=1
    ...
    def make_bad_channels_selection(
        ...
        **state,
        ...
        raw_data = self._load_derivative(
            raw_node_name(raw_name), options={"preload": False, "noise": False}
        )
        ...
        bads_ctx = self._resolve_derivative(
            raw_bad_channels_input_name(source_name)
        )
        ...
        # Sensor system info
        source_pipe = self._raw.root_source_pipe(raw_name)
        data_kind = DataSpec("sensor").find_ndvar_channel_types(raw_data.info)[
            0
        ]
        sysname = source_pipe._get_sysname(raw_data.info, subject, data_kind)
        adjacency = source_pipe._get_adjacency(data_kind)
        return gui.select_channels(
            raw_data,
            channels_path,
            events=events,
            sysname=sysname,
    ...
    def make_epoch_rejection(
        ...
        **state,
        ...
        if rej_args is None:
            raise ValueError(
                f"epoch_rejection={rej!r}; no epoch rejection configured"
            )
        ...
        if not isinstance(epoch, PrimaryEpoch):
            if isinstance(epoch, SecondaryEpoch):
                raise ValueError(
                    f"The current epoch {epoch.name!r} inherits selections from {epoch.sel_epoch!r}. To access a rejection file for this epoch, call `e.set(epoch={epoch.sel_epoch!r})` and then call `e.make_epoch_rejection()` again."
                )
            elif isinstance(epoch, SuperEpoch):
                raise ValueError(
                    f"The current epoch {epoch.name!r} inherits selections from these other epochs: {epoch.sub_epochs!r}. To access selections for these epochs, call `e.make_epoch_rejection(epoch=epoch)` for each."
                )
            else:
                raise ValueError(
                    f"The current epoch {epoch.name!r} is not a primary epoch and inherits selections from other epochs. Generate trial rejection for these epochs."
                )
        ...
        if isinstance(rej_args, (ChannelModelRejection, RANSACRejection)):
            ...
            node = (
                "epoch-rejection-ransac"
                if isinstance(rej_args, RANSACRejection)
                else "epoch-rejection-channel-model"
            )
            rej_ctx = self._resolve_derivative(node)
            rej_ctx.load()
            path = rej_ctx.node.path(rej_ctx)
            ds = self._load_derivative(
                "epochs", options={"reject": False, "ndvar": False}
            )
            return gui.select_epochs(
                ds, "epochs", trigger="value", path=path, read_only=True
            )
        elif not isinstance(rej_args, ManualRejection):
            raise NotImplementedError(
                f"make_epoch_rejection for {type(rej_args).__name__}"
            )
        ...
        if auto is not None and overwrite is not True and path.exists():
            if overwrite is False:
                ...
                raise OSError(
                    self.format(
                        "A rejection file already exists for {subject}, epoch {epoch}, rej {rej}. Set the overwrite parameter to specify how to handle existing files."
                    )
                )
            else:
        ...
        if auto is not None:
            ds = self._load_derivative(
                "epochs", options={"reject": False, "ndvar": True}
            )
            ...
            if isinstance(auto, dict):
                ...
                if missing := set(ch_types).difference(auto_dict):
                    raise ValueError(
                        f"{auto=}: channel types {enumeration(missing)} missing"
                    )
                elif unknown := set(auto_dict).difference(ch_types):
                    raise ValueError(
                        f"{auto=}: channel types {enumeration(unknown)} not in data"
                    )
            elif len(ch_types) == 1:
            ...
            if decim is not None:
                args.append(f"{decim=}")
            rej_ds.info["desc"] = (
                f"Created with {self.__class__.__name__}.make_epoch_rejection({', '.join(args)})"
            )
            # save
            ...
            self._log.info(
                f"make_epoch_rejection: {n_rej} of {rej_ds.n_cases} epochs rejected with threshold {auto} for {desc}"
            )
            return
    ...
    def next(self, field: str | Sequence[str] = "subject"):
        ...
        if idx == -1:
            next_ = values[0]
            print(
                f"The last {fmt(field)} was reached; rewinding to {fmt(next_)}"
            )
        else:
            next_ = values[idx]
        ...
        **state,
        ...
        if seeds:
            if not isinstance(parc, SeededParc):
                raise ValueError(
                    f"seeds=True is only valid for seeded parcellation, not for parc={parc_name!r}"
                )
            # if seeds are defined on a scaled common-brain, we need to plot the scaled brain:
            plot_on_scaled_common_brain = isinstance(
                parc, IndividualSeededParc
            )
        else:
        ...
        state_ = self._fields
        subjects_dir = str(self.root / MRI_SDIR)
        if (not plot_on_scaled_common_brain) and is_fake_mri(
            self.root / mri_dir(state_)
        ):
            subject = self.get("common_brain")
        else:
            subject = self.get("mrisubject")

        kwa = self._surfer_plot_kwargs(
            surf, views, foreground, background, None, hemi
        )
        brain = plot.brain.annot(
            parc_name,
            subject,
            borders=borders,
            alpha=alpha,
        ...
        if seeds:
            ...
            seed_points = {
                hemi: [
                    ...
                    if name.endswith(hemi)
                ]
                for hemi in ("lh", "rh")
            }
            plot_points = {
                hemi: np.vstack(points).T if len(points) else None
    ...
    def plot_coregistration(
        ...
        **state,
        ...
        fig = mne.viz.plot_alignment(
            raw.info,
            self.root / trans_file_path(state_, datatype=self._datatype),
            self.get("mrisubject"),
            self.root / MRI_SDIR,
            surfaces,
            meg=meg,
    ...
    def plot_whitened_gfp(self, s_start=None, s_stop=None, run=None):
        ...
        with self._temporary_state:
            for subject in self.iter_range(s_start, s_stop):
                ...
                whitened_evoked = mne.whiten_evoked(
                    ds[0, "evoked"], cov, picks
                )
    ...
    def plot_evoked(
        ...
        **kwargs,
        ...
        source_inv = self.get(
            "inv"
        )  # source space requires a configured inverse
        if data is None:
            sns = True
            src = bool(
                source_inv
            )  # only plot source estimates if an inverse is configured
        else:
            data = DataSpec.coerce(data)
            sns, src = bool(data.sensor), bool(data.source)
            if src and not source_inv:
                raise ValueError(
                    f"data={data.string!r}: no inverse is configured (inv=''); set inv to plot source estimates"
                )
        # response NDVar key(s) for the sensor plots are named by a DataSpec
        sensor_data = (
            data if isinstance(data, DataSpec) else DataSpec("sensor")
        )
        model = self._eval_model(model)
        epoch = self.get("epoch")
        if model:
        ...
        if is_vector_data and not is_volume_source_space:
            raise NotImplementedError(
                "Vector data currently can only be plotted for volume source space"
            )
        ...
        if separate and not subject:
            ...
            for subject in self.iter(group=group):
                ...
                p = plot.TopoButterfly(
                    y, model or None, data=ds, axh=h, name=title, run=False
                )
                plots.append(p)
        ...
        if src:
            ...
            for y in ys:
                if is_volume_source_space:
                    plots = plot.GlassBrain.butterfly(
                        y, w=2 * h, h=h, name=title
                    )
                else:
            ...
            ds = self.load_evoked(
                subject_arg, baseline=baseline, inv="", model=model
            )
            out = [ds]
            right_of = None
        if sns:
    ...
    def plot_raw(self, decim=10, xlim=5, subtract_mean=False, **state):
        ...
        state_ = self._fields
        name = join_stem_parts(
            raw_basename(state_, datatype=self._datatype),
            f"raw-{state_['raw']}",
        )
        if raw.info["meas"] == "V":
            vmax = 1.5e-4
        ...
        if subtract_mean:
            raw -= raw.mean("time")
        return plot.TopoButterfly(
            raw, w=0, h=3, xlim=xlim, vmax=vmax, name=name
        )
    ...
    def set(self, subject: str = None, **state):
        ...
        if subject is not None:
            if "group" not in state:
                if (
                    subject not in self._field_values["subject"]
                    and subject in self._groups["all"]
                ):
                    old = self.get("group")
    ...
    def set_inv(
        ...
        **state,
        ...
        """
        self.set(
            inv=self.inv_str(ori, snr, method, depth, pick_normal), **state
        )
    ...
    def inv_str(
        ...
        return MinimumNormInverseSolution(
    ...
    def _eval_model(self, model: str) -> str:
        if model == "":
            ...
            raise ValueError(
                f"{model=}; To specify interactions, use '%' instead of '*'"
            )
    ...
    def _update_mrisubject(self, fields: dict) -> str:
        ...
        if mrisubject == self.get("common_brain") or mrisubject.startswith(
            "sub-"
        ):
            return mrisubject
    ...
    def _update_run(self, fields: dict) -> str | None:
        ...
        if runs and fields.get("run") not in runs:
            return runs[
                0
            ]  # current run invalid for this subject/session/task; reset
    ...
    def _update_acquisition(self, fields: dict) -> str | None:
        ...
        acquisitions = sorted(
            {
                acquisition
                for subject_, session_, task_, acquisition, _ in self._recordings
                if (subject_, session_, task_) == (subject, session, task)
            }
        )
    ...
    def _eval_parc(self, parc: str) -> str:
        ...
        if parc in self._parcs:
            if isinstance(self._parcs[parc], SeededParc):
                raise ValueError(
                    f"Seeded parc set without size, use e.g. parc='{parc}-25'"
                )
            else:
                return parc
        ...
        if m:
            ...
            if isinstance(self._parcs.get(name), SeededParc):
                ...
                raise ValueError(
                    f"{parc=}: No parcellation named '{parc}' and no seeded parcellation named '{name}'"
                )
        else:
    ...
    def show_bad_channels(
        ...
        **state,
        ...
        if use_tasks is None:
            bad_channels = {
                subject: self.load_bad_channels() for subject in self
            }
            list_tasks = False
        else:
            bad_channels = {
                key: self.load_bad_channels()
                for key in self.iter(
                    ("subject", "task"), values={"task": use_tasks}
                )
            }
    ...
    def _show_dependencies(
        ...
        **state,
        ...
        """
        self.set(**state)
        tree = self._derivatives.dependency_tree(
            name,
            state=self.state,
            options=options,
    ...
    def show_head_position_overview(
        ...
        **state,
        ...
        for subject, session, task, run in self.iter(
            ("subject", "session", "task", "run"), raw="raw"
        ):
            key = (task, run)
            ctx = self._resolve_derivative(node_name, options={"noise": False})
            if ctx.exists():
                data.setdefault(session, {}).setdefault(subject, {})[key] = (
                    None
                )
                chl.setdefault(session, {}).setdefault(subject, {})[key] = (
                    False
                )
                info = self._load_derivative(
                    node_name, view="info", options={"noise": False}
                )
        ...
        def _row_keys(session: str) -> list[tuple[str, str]]:
            ...
            found = {
                key
                for subj_data in data[session].values()
                for key in subj_data
            }
            return sorted(
                found, key=lambda tr: (task_order.get(tr[0], 999), tr[1])
            )
            ...
            for subject in subjects:
                ...
                for key in row_keys:
                    ...
                    for rep_label, rep_trans in representatives:
                        if np.allclose(
                            trans, rep_trans, atol=tolerance, rtol=0
                        ):
        ...
        def _make_caption(session: str) -> str:
            ...
            if len(row_keys) > 1:
                ...
                patterns = [
                    tuple(
                        session_labels[s].get(key, MISSING_MARK)
                        for key in row_keys
                    )
                    for s in subjects
                ]
                ...
                if majority_n == len(subjects):
                    if set(majority_pat) == {"A"}:
                        parts.append(
                            "All subjects have the same head position for all tasks."
                        )
                    else:
            ...
            if any_missing:
                parts.append(
                    f"{MISSING_MARK}: no initial head position (dev_head_t)."
                )
            if any_chl:
        ...
        def _make_table(session: str, title: str = None) -> fmtxt.Table:
            ...
            t = fmtxt.Table(
                col_spec, title=title, caption=_make_caption(session)
            )
            t.cell("Task")
            if has_runs:
                t.cell("Run")
        ...
        def _session_sig(session: str) -> dict[str, tuple]:
            ...
            row_keys = _row_keys(session)
            return {
                s: tuple(sl[s].get(key, MISSING_MARK) for key in row_keys)
                for s in subjects
            }
        ...
        if all(_session_sig(ses) == sig0 for ses in sessions[1:]):
            return _make_table(
                sessions[0],
                title="Head position (identical across all sessions)",
            )
    ...
    def show_raw_info(self, **state) -> fmtxt.Table | None:
        ...
        print(
            f"Preprocessing pipeline: {' --> '.join(p.name for p in pipeline)}"
        )
    ...
    def show_rej_info(self, flagp=None, asds=False, bads=False, **state):
        ...
        if bads_in_rej:
            ...
            bad_chs = [
                f"{bads_raw} + {bads_rej}" for bads_raw, bads_rej in bad_chs
            ]
        else:
        ...
        if has_epoch_rejection:
            ...
            out["percent"] = Var(
                np.round(100 * out["n_good"] / out["n_events"], 1)
            )
        if flagp:
            out["flag"] = Factor(
                out["percent"] < flagp, labels={False: "", True: "*"}
            )

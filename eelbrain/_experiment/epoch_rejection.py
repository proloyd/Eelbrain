...
class BadWindowsRejectionDerivative(Derivative[Dataset]):
    ...
    def dependencies(self, ctx: Request) -> tuple[Dependency, ...]:
        ...
        return (
            Dependency(
                "epochs", label="score-epochs", options=self._EPOCH_OPTIONS
            ),
            Dependency(
                clean_windows_input_name(rej.raw),
                label="clean-windows",
                state={"raw": rej.raw, "task": epoch.task},
            ),
            Dependency(
                raw_node_name(ctx.state["raw"]),
                label="scoring-raw-info",
                view="info",
    ...
    def fingerprint(self, ctx: Request) -> dict[str, Any]:
        return {
            "epoch_rejection": self.epoch_rejection[
    ...
    def build(self, ctx: Request) -> Dataset:
        ...
        if "eeg" not in score_ds:
            raise ConfigurationError(
                f"epoch_rejection={ctx.state['epoch_rejection']!r}: BadWindowsRejection requires EEG data, but {ctx.state['subject']}/{ctx.state['epoch']} has none"
            )
        eeg = score_ds["eeg"]
        ...
        if isinstance(eeg, Datalist):
            ...
            windows = [
                bad_intervals_to_windows(
                    bad_intervals, samples[i], raw_sfreq, tmin, tmax
                )
                for i in range(score_ds.n_cases)
            ]
...
class RejectionInput(Input):
    ...
    def path(self, ctx: Request) -> Path:
        ...
        if not isinstance(epoch, PrimaryEpoch):
            raise RuntimeError(f"{epoch=}")
        return ctx.root / rej_file_path(
            ctx.state, epoch=epoch.name, datatype=ctx.datatype
        )
...
class ChannelModelRejectionDerivative(Derivative[Dataset]):
    ...
    def build(self, ctx: Request) -> Dataset:
        ...
        if "eeg" not in score_ds:
            raise ConfigurationError(
                f"epoch_rejection={ctx.state['epoch_rejection']!r}: ChannelModelRejection requires EEG data, but {ctx.state['subject']}/{ctx.state['epoch']} has none"
            )
        eeg = score_ds["eeg"]
        ...
        if continuous:
            rej_ds = new_rejection_ds(score_ds, windows=True)
            rej_ds.info[INTERPOLATE_WINDOWS_MAX] = rej.max_interpolate
            rej_ds[INTERPOLATE_WINDOWS] = model.find_bad_windows(
                eeg,
                threshold=rej.score_threshold,
                max_exclude=rej.max_interpolate,
                window=rej.window,
                hop=rej.hop,
                min_duration=rej.min_duration,
                merge_gap=rej.merge_gap,
            )
            return rej_ds
        ...
        scores = model.score(
            eeg,
            threshold=rej.score_threshold,
            max_exclude=rej.max_interpolate + 1,
        )
        ...
        for i in range(score_ds.n_cases):
            bad = [
                names[j]
                for j in np.flatnonzero(score_data[i] > rej.score_threshold)
            ]
...
class RANSACRejectionDerivative(Derivative[Dataset]):
    ...
    def build(self, ctx: Request) -> Dataset:
        ...
        if "eeg" not in score_ds:
            raise ConfigurationError(
                f"epoch_rejection={ctx.state['epoch_rejection']!r}: RANSACRejection requires EEG data, but {ctx.state['subject']}/{ctx.state['epoch']} has none"
            )
        eeg = score_ds["eeg"]
        if self._separate_fit_raw(ctx) is not None:
            fit_eeg = ctx.load("fit-epochs")["eeg"]
            ...
            fit_eeg = eeg
        model = ChannelRANSACModel(
            window_len=rej.window_len,
            n_resamples=rej.n_resamples,
            subset_size=rej.subset_size,
            random_seed=rej.random_seed,
            n_jobs=rej.n_jobs,
        )
        model.fit(fit_eeg)
        ...
        if continuous:
            ...
            rej_ds[INTERPOLATE_WINDOWS] = model.find_bad_windows(
                eeg,
                window_len=rej.window_len,
                corr_threshold=rej.corr_threshold,
                min_duration=rej.min_duration,
                merge_gap=rej.merge_gap,
            )
            return rej_ds

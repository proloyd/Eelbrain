...
class BadWindowsRejectionDerivative(Derivative[Dataset]):
    ...
    def build(self, ctx: Request) -> Dataset:
        ...
        if isinstance(eeg, Datalist):
            windows = [
                bad_intervals_to_windows(bad_intervals, samples[i], raw_sfreq, eeg[i].time.tmin, eeg[i].time.tstop)
                for i in range(score_ds.n_cases)
            ]
        else:
            tmin, tmax = eeg.time.tmin, eeg.time.tstop
            windows = [
                bad_intervals_to_windows(bad_intervals, samples[i], raw_sfreq, tmin, tmax)
                for i in range(score_ds.n_cases)
            ]

...
from ..._meeg.interpolation import (
    _interpolate_bads_eeg,
    _interpolate_bads_meg,
    _interpolate_bad_windows_eeg,
    _interpolate_bad_windows_meg,
)
...
def _load_epochs(path: Path, metadata: dict[str, Any]):
    if metadata["kind"] == "datalist":
        return Datalist(
            [
                mne.read_epochs(path / relpath, proj=False)
                for relpath in metadata["files"]
            ],
            metadata["name"],
            metadata["fmt"],
        )
...
class RecordingEpochsDerivative(Derivative[Any]):
    ...
    def dependency_fingerprint_override(
        self, ctx: Request, dep: Dependency, dep_ctx: Request
    ) -> dict[str, Any] | None:
        ...
        if (
            ds.info.get(INTERPOLATE_CHANNELS, False)
            and INTERPOLATE_CHANNELS in ds
        ):
            out[INTERPOLATE_CHANNELS] = ds[INTERPOLATE_CHANNELS]
        if (
            ds.info.get(INTERPOLATE_WINDOWS, False)
            and INTERPOLATE_WINDOWS in ds
        ):
    ...
    def build(self, ctx: Request):
        ...
        if ds.info[BAD_CHANNELS]:
            raw.info["bads"] = sorted(
                set(raw.info["bads"] + ds.info[BAD_CHANNELS])
            )
        ds.info["raw"] = raw
        tmin, tmax, tstop, decim, variable_tmax = epoch._extraction_parameters(
            ds, ctx.options
        )
        ...
        if variable_tmax:
            epochs_list = load.mne.variable_length_mne_epochs(
                ds,
                tmin,
                tmax,
                None,
            ...
            if epoch.post_baseline_trigger_shift:
                ...
                if isinstance(epochs, Datalist):
                    raise NotImplementedError(
                        "post_baseline_trigger_shift for variable-length SuperEpoch"
                    )
                if epoch.baseline:
                    epochs.apply_baseline(epoch.baseline)
                shift = ds.eval(epoch.post_baseline_trigger_shift)
                epochs = shift_mne_epoch_trigger(
                    epochs,
                    shift,
        ...
        data_types = DataSpec("sensor").find_ndvar_channel_types(
            epochs_list[0].info
        )
        if ds.info.get(INTERPOLATE_WINDOWS, False) and any(
            ds[INTERPOLATE_WINDOWS]
        ):
            ...
            for epochs in epochs_list:
                ...
                if "eeg" in data_types:
                    _interpolate_bad_windows_eeg(
                        epochs, windows, max_interpolate
                    )
            ...
            if ds.info[INTERPOLATE_CHANNELS] and any(ds[INTERPOLATE_CHANNELS]):
                ...
                bads_individual = [
                    sorted(set(bads_all + bads_i))
                    for bads_i in ds[INTERPOLATE_CHANNELS]
                ]
                if "mag" in data_types:
                    interp_cache = {}
                    _interpolate_bads_meg(
                        epoch_value, bads_individual, interp_cache
                    )
                if "eeg" in data_types:
        ...
        if reference is not None:
            if "eeg" not in DataSpec("sensor").find_ndvar_channel_types(
                epochs_list[0].info
            ):
                raise ConfigurationError(
                    f"reference={ctx.state['reference']!r}: {ctx.state['subject']}/{epoch.name} has no EEG channels to re-reference; set reference='' for data without EEG."
                )
            montage = self.raw.root_source_pipe(ctx.state["raw"]).montage
            epochs_list = [
                reference._apply_reference(epochs, montage=montage)
                for epochs in epochs_list
            ]
            epoch_value = (
                Datalist(epochs_list, "epochs")
                if variable_tmax
                else epochs_list[0]
            )
...
class EpochsDerivative(UncachedDerivative[Dataset]):
    ...
    def dependencies(self, ctx: Request) -> tuple[Dependency, ...]:
        ...
        if isinstance(epoch, EpochCollection):
            raise TypeError(
                f"{epoch=}: load_epochs not supported for EpochCollection"
            )
        if isinstance(epoch, SuperEpoch):
            ...
            epoch_overrides = {
                k: getattr(epoch, k)
                for k in epoch._explicit_params
                if k in epoch.INHERITED_PARAMS
            }
            forward_keys = [
                k
                for k in (*EPOCH_EXTRACT_OPTIONS, "interpolate_bads", "reject")
                if k not in epoch_overrides
            ]
        ...
        rec_options = ctx.options_for(
            "recording-epochs", *RecordingEpochsDerivative.key_options
        )
        sel_options = ctx.options_for(
    ...
    def build(self, ctx: Request) -> Dataset:
        ...
        if not data.sensor:
            raise ValueError(
                f"data={data.string!r}; load_evoked is for loading sensor data"
            )
        ...
        if isinstance(epoch, SuperEpoch):
            ...
            for sub_epoch in epoch.sub_epochs:
                ...
                if epoch.post_baseline_trigger_shift:
                    ...
                    shift = ds.eval(epoch.post_baseline_trigger_shift)
                    epoch_value = shift_mne_epoch_trigger(
                        epoch_value,
                        shift,
                        epoch.post_baseline_trigger_shift_min,
                        epoch.post_baseline_trigger_shift_max,
            ...
            runs = self._find_runs(ctx, epoch)
            if runs:
                epoch_value = Datalist(
                    [ctx.load(f"epochs-{run}") for run in runs], "epochs"
                )
            else:
                ...
                bads = sorted(
                    {
                        channel
                        for epochs in epochs_list
                        for channel in epochs.info["bads"]
                    }
                )
            for epochs in epochs_list:
        ...
        if epoch.post_baseline_trigger_shift:
            if baseline is not True and baseline != epoch.baseline:
                raise NotImplementedError(
                    f"{baseline=} for epoch {epoch.name!r}: baseline correction is applied before the post_baseline_trigger_shift and can not be changed at load time; use baseline=True"
                )
        else:
            if baseline is True:
        ...
        if ndvar:
            ...
            for data_kind in sensor_types:
                sysname = source_pipe._get_sysname(
                    info, ds.info["subject"], data_kind
                )
                adjacency = source_pipe._get_adjacency(data_kind)
                if variable_tmax:
                    ys = Datalist(
                        [
                            load.mne.epochs_ndvar(
                                epochs,
                                data=data_kind,
                                sysname=sysname,
                                adjacency=adjacency,
                    ...
                    if data.aggregate:
                        ys = Datalist(
                            [getattr(y, data.aggregate)("sensor") for y in ys]
                        )
                else:
                    ys = load.mne.epochs_ndvar(
                        ds["epochs"],
                        data=data_kind,
                        sysname=sysname,
                        adjacency=adjacency,
...
class EvokedDerivative(Derivative[list[mne.Evoked]]):
        ...
        return (
            ...
            Dependency(
                ...
                options=ctx.options_for(
                    "epoch-events", "samplingrate", "decim", reject=True
                ),
            ),
        )
    ...
    def dependency_fingerprint(
        ...
        if view != "shell":
            raise ValueError(
                f"{self.name!r} does not define dependency view {view!r}"
            )
        return self.fingerprint(ctx)
    ...
    def apply_view_options(
        self, ctx: Request, evoked: list[mne.Evoked]
    ) -> Dataset:
        ...
        model_vars = model.split("%") if model else ()
        cells = (
            [" | ".join(cell) or "No comment" for cell in ds.zip(*model_vars)]
            if model_vars
            else ["No comment"]
        )
        ...
        if len(evoked_by_cell) != len(evoked):
            raise RuntimeError(
                f"Cached evoked data contains duplicate comments: {_evoked_comments(evoked)!r}"
            )
        try:
            evoked = [evoked_by_cell[cell] for cell in cells]
        except KeyError:
            raise RuntimeError(
                f"Error reading cached evoked: available={tuple(evoked_by_cell)}, requested={tuple(cells)}"
            ) from None
        ...
        if epoch.post_baseline_trigger_shift:
            if baseline is not True and baseline != epoch.baseline:
                raise NotImplementedError(
                    f"baseline={baseline!r} for epoch {epoch.name!r}: baseline correction is applied before the post_baseline_trigger_shift and can not be changed at load time; use baseline=True"
                )
        else:
            if baseline is True:
                baseline = epoch.baseline
        ...
        if to_ndvar:
            info = evoked[0].info
            sensor_types = ds.info["sensor_types"] = (
                data.find_ndvar_channel_types(info)
            )
            source_pipe = self.raw.root_source_pipe(ctx.state["raw"])
            for sensor_type in sensor_types:
                sysname = source_pipe._get_sysname(
                    info, ctx.state["subject"], sensor_type
                )
                adjacency = source_pipe._get_adjacency(sensor_type)
                ds[sensor_type] = load.mne.evoked_ndvar(
                    evoked,
                    data=sensor_type,
                    sysname=sysname,
                    adjacency=adjacency,
                ...
                if sensor_type != "eog" and data.aggregate:
                    ds[sensor_type] = getattr(ds[sensor_type], data.aggregate)(
                        "sensor"
                    )
...
class EvokedGroupDatasetDerivative(UncachedDerivative[Dataset]):
    ...
    def build(self, ctx: Request) -> Dataset:
        dss = [
            ctx.load(subject) for subject in self.groups[ctx.state["group"]]
        ]
        data = ctx.options["data"]

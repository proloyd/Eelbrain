...
def canonical_recording(
    ...
    matches = sorted(
        (task, run)
        for subject_, session_, task, acquisition_, run in recordings
        if subject_ == subject
        and session_ == (session or "")
        and acquisition_ == (acquisition or "")
    )
...
class RawBadChannelsInput(Input[list[str]]):
    ...
    def path(self, ctx: Request) -> Path:
        ...
        return bpath.update(
            root=ctx.root / DERIV_DIR / "mne", check=False
        ).fpath
    ...
    def dependency_fingerprint_quick(
        self, ctx: Request, view: str | None = None
    ) -> dict[str, Any] | None:
    ...
    def load(self, ctx: Request) -> list[str]:
        ...
        if "name" not in channels_df.columns:
            raise RuntimeError(
                f"channels.tsv file at {path} is missing required column 'name'."
            )
        ...
        *,
        ...
        if path.exists():
            ...
            if source_path.exists():
                LOG.info(
                    "No bad-channels file found at %s, seeding from BIDS source %s.",
                    path,
                    source_path,
                )
                ...
                if "name" not in channels_df.columns:
                    raise RuntimeError(
                        f"channels.tsv file at {source_path} is missing required column 'name'."
                    )
                if "status" not in channels_df.columns:
                    channels_df["status"] = "good"
            else:
                LOG.info(
                    "No bad-channels file found at %s, creating one from raw.",
                    path,
                )
                ch_status = [
                    "bad" if ch in raw.info["bads"] else "good"
                    for ch in raw.ch_names
                ]
                channels_df = pd.DataFrame(
                    {"name": raw.ch_names, "status": ch_status}
                )
            path.parent.mkdir(parents=True, exist_ok=True)
            created = True
        else:
            raise FileMissingError(
                f"Bad channels file does not exist at {path}"
            )
        ...
        if missing:
            raise RuntimeError(
                f"channels.tsv file at {path} is missing bad channel names: {missing!r}."
            )
        if redo:
            channels_df["status"] = "good"
...
class RawSourceInput(Input[mne.io.BaseRaw]):
    ...
    def _resolve_bids_path(
        self, ctx: Request, require: bool = False
    ) -> BIDSPath:
        """Return the noise-resolved BIDSPath and the actual file path on disk."""
        bids_path_ = bids_path(
            ctx.root, ctx.state, self.extension, datatype=ctx.datatype
        )
        ...
        if require:
            raise FileMissingError(
                f"Raw input file does not exist at expected location {bids_path_.fpath}"
            )
        return bids_path_
    ...
    def _read_raw(path: BIDSPath, preload: bool) -> mne.io.BaseRaw:
        ...
        match path.extension:
            ...
            case _:
                raise RuntimeError(
                    f"Unrecognized file format: {path.extension}"
                )
    ...
    def _load_raw(self, ctx: Request, preload: bool):
        ...
        if self.pipe.rename_channels:
            if rename := {
                k: v
                for k, v in self.pipe.rename_channels.items()
    ...
    def _find_bids_channels(path: BIDSPath) -> Path | None:
        ...
        channels_path = path.find_matching_sidecar(
            suffix="channels", extension=".tsv", on_error="ignore"
        )
    ...
    def _apply_bids_channels(path: BIDSPath, raw: mne.io.BaseRaw) -> None:
        ...
        if "name" not in channels_df.columns:
            warnings.warn(
                f"{channels_path} has no 'name' column; skipping channel metadata."
            )
            return
        ...
        if "type" in channels_df.columns:
            ...
            for ch_name, ch_type in zip(
                channels_df["name"], channels_df["type"]
            ):
                ...
                if updated_ch_type is None:
                    ...
                    warnings.warn(
                        f"No BIDS -> MNE mapping found for channel type {ch_type_bids!r}. Type of channel {ch_name!r} will be set to 'misc'."
                    )
    ...
    def _find_bids_electrodes(path: BIDSPath) -> tuple[Path, Path] | None:
        ...
        space_candidates = sorted(
            data_dir.glob(f"{sub_prefix}_space-*_electrodes.tsv")
        )
        if space_candidates:
            if len(space_candidates) > 1:
                warnings.warn(
                    f"Multiple electrodes.tsv files found in {data_dir}; using {space_candidates[0].name}"
                )
            elec_path = space_candidates[0]
        else:
            elec_path = (
                path.copy().update(suffix="electrodes", extension=".tsv").fpath
            )
            if not elec_path.exists():
                return None
        coord_path = elec_path.with_name(
            elec_path.name.replace("_electrodes.tsv", "_coordsystem.json")
        )
    ...
    def _apply_bids_electrodes(path: BIDSPath, raw: mne.io.BaseRaw) -> None:
        ...
        if not coord_path.exists():
            warnings.warn(
                f"No matching coordsystem.json found for {elec_path.name}; electrode positions not applied."
            )
            return
        ...
        if coord_frame is None:
            warnings.warn(
                f"Unrecognized EEG coordinate system {coord_frame_bids!r} in {coord_path.name}; electrode positions not applied."
            )
            return
        scale = COORD_SCALE.get(coord_unit)
        if scale is None:
            warnings.warn(
                f"Unrecognized EEG coordinate unit {coord_unit!r} in {coord_path.name}; electrode positions not applied."
            )
            return
        elec_df = pd.read_csv(elec_path, sep="\t")
        numeric = elec_df[["x", "y", "z"]].apply(
        ...
        montage = mne.channels.make_dig_montage(
            ch_pos=ch_pos, coord_frame=coord_frame
        )
...
class RawSourceDerivative(UncachedDerivative[mne.io.BaseRaw]):
    ...
    def dependencies(self, ctx: Request) -> tuple[Dependency, ...]:
        ...
        return (
            Dependency(
                source_name,
                options=ctx.options_for(source_name, "noise", preload=False),
            ),
    ...
    def apply_view_options(
        self, ctx: Request, raw: mne.io.BaseRaw
    ) -> mne.io.BaseRaw:
    ...
    def load_view(self, ctx: Request, view: str):
        ...
        if view == "info":
            info = ctx.load(
                source_name,
                options=ctx.options_for(source_name, "noise"),
                view="info",
            )
            with info._unlock():
    ...
    def _load_bad_channels(self, ctx: Request) -> list[str]:
        ...
        nan_bads = {
            raw.info["chs"][i]["ch_name"]
            for i in eeg_picks
            if numpy.any(numpy.isnan(raw.info["chs"][i]["loc"][:3]))
        }
        ...
        if nan_bads:
            ...
            if eeg_names and eeg_names.issubset(nan_bads):
                raise DataError(
                    "All EEG channel positions are NaN. This usually means that the raw file does not contain electrode positions and a montage needs to be applied. Set the montage parameter in RawSource to supply channel positions."
                )
            warnings.warn(
                f"Channels with NaN position marked as bad: {', '.join(sorted(nan_bads))}",
                RuntimeWarning,
            )
            all_bads |= nan_bads
...
class ICAInput(Input[mne.preprocessing.ICA]):
    ...
    def _key(self, ctx: Request) -> dict[str, Any]:
        return canonical_state_subset(
            {**ctx.state, "raw": self.raw_name}, self.key_fields
        )

    def _manifest(self, ctx: Request) -> ArtifactManifest | None:
        return ctx.registry.read_manifest(
            ctx.registry.manifest_path(self.path(ctx), self.name)
        )
    ...
    def _source_states(
        self, ctx: Request, tasks: Sequence[str]
    ) -> list[dict[str, str]]:
    ...
    def load_concatenated_source_raw(
        ...
        if not states:
            raise FileMissingError(
                f"No source recordings found to estimate ICA {self.raw_name!r} ({ctx.state['subject']=}, session={ctx.state.get('session')!r})."
            )
        raw = load_raw_dependency(
            ctx, self.pipe.source, preload=True, state=states[0]
        )
        raw.info["bads"] = bad_channels
        for state in states[1:]:
            raw_ = load_raw_dependency(
                ctx, self.pipe.source, preload=True, state=state
            )
    ...
    def _reindex_existing(self, ctx: Request) -> mne.preprocessing.ICA:
        ...
        ctx.registry.write_manifest(
            ctx.registry.manifest_path(self.path(ctx), self.name),
            self._build_manifest(ctx, value),
        )
        return value
    ...
    def _stale_reason(
        ...
        diff = find_difference(
            previous.fingerprint.get("pipe"),
            current.fingerprint.get("pipe"),
            coarsen=False,
        )
        if diff is not None:
            path, old, new = diff
        ...
        diff = find_difference(
            previous.dependencies, current.dependencies, strip_quick=True
        )
        if diff is not None:
            path, old, new = diff
            dep = path[0]
            if dep.endswith(":raw"):
                    ...
                    def _fmt_mtime(v: Any) -> str:
                        t = (
                            ...
                            if isinstance(v, (int, float))
                                ...
                                v.get("mtime") if isinstance(v, dict) else None
                            )
                        )
                        return (
                ...
                field = self._format_pipe_setting(
                    path[1:], ("fingerprint", "pipe")
                )
        ...
        if diff is not None:
            ...
            if path == ("bads",):
                ...
                lines = [
                    "The set of bad channels used for ICA estimation changed."
                ]
                if shared:
    ...
    def _dependency_raw_name(
        ...
        previous_dep = (
            {}
            if previous is None
            else previous.dependencies.get(dependency, {})
        )
    ...
    def is_valid(self, ctx: Request) -> bool:
        ...
        if not path.exists():
            return False
        return self._manifest_matches(
            self._manifest(ctx), self._current_value_manifest(ctx)[1]
        )
    ...
    def load(self, ctx: Request) -> mne.preprocessing.ICA:
        ...
        if not path.exists():
            raise FileMissingError(
                f"ICA file {path.name} does not exist. Run e.make_ica() to create it."
            )
        ...
        if not self._manifest_matches(previous, current):
            if ctx.has_control(REINDEX_ICA):
                ctx.registry.write_manifest(
                    ctx.registry.manifest_path(path, self.name), current
                )
                return value
            ...
            raise ProtectedArtifactError(
                self.name,
                path,
                message=f"Existing ICA file {path.name!r} no longer matches the current data and ICA settings.",
                reason=reason,
    ...
    def materialize(
        ...
        ctx.registry.write_manifest(
            ctx.registry.manifest_path(path, self.name),
            self._build_manifest(ctx, value),
        )
...
class RawDerivative(Derivative[mne.io.BaseRaw]):
    ...
    def dependencies(self, ctx: Request) -> tuple[Dependency, ...]:
        ...
        if isinstance(self.pipe, RawICA):
            ...
            deps.append(
                Dependency(ica_node, view="bads", label=f"{ica_node}:bads")
            )
            if ctx.options["noise"]:
                deps.append(
                    Dependency(
            ...
            deps.append(
                Dependency(raw_node_name(self.pipe.ica_source), view="bads")
            )
    ...
    def dependency_fingerprint(
        ...
        if view == "bads":
            return {
                "raw": self.raw_name,
                "pipe": self.pipe,
                "bads": self.pipe._collect_bads(
                    ctx, noise=ctx.options["noise"]
                ),
            }
    ...
    def build(self, ctx: Request) -> mne.io.BaseRaw:
        ...
        if isinstance(self.pipe, (RawICA, RawApplyICA)):
            ...
            if isinstance(self.pipe, RawICA):
                ...
                bad_channels = sorted(
                    set(ctx.load(f"{source_node}:bads"))
                    | set(ctx.load(raw_node_name(self.pipe.ica_source)))
                )
            return ica_pipe._apply_ica(
                raw, ica, bad_channels, self.raw_name, log=ctx.registry.log
            )
    ...
    def load(self, ctx: Request, path: Path) -> mne.io.BaseRaw:
        with warnings.catch_warnings():
            ...
            raw = mne.io.read_raw_fif(
                path, preload=False, verbose=MNE_VERBOSITY
            )
        return raw
    ...
    def load_view(self, ctx: Request, view: str):
        ...
        path = bids_path(
            ctx.root, state, self.extension, datatype=ctx.datatype
        )
        upstream_info = load_raw_info_dependency(
            ctx, self.pipe.source, noise=ctx.options["noise"]
        ).copy()
        info = self.pipe._make_info(
            upstream_info,
            path=path,
            noise=ctx.options["noise"],
            raw_name=self.raw_name,
        ...
        with info._unlock():
            info["bads"] = self.pipe._collect_bads(
                ctx, noise=ctx.options["noise"]
            )
        return info
...
_CLEAN_WINDOWS_EVENTS_JSON = {
    ...
    "channel": {
        "Description": "Name of the channel that was bad in this window (clean_windows does not score channels jointly, so each channel's bad windows are reported as a separate row)."
    },
}
...
class CleanWindowsDerivative(Derivative[list]):
    ...
    def _description(self) -> str:
        ...
        return "cleanwindows" + "".join(
            c for c in self.raw_name if c.isalnum()
        )

    def path(self, ctx: Request) -> Path:
        bpath = bids_path(
            ctx.root, ctx.state, ".tsv", datatype=ctx.datatype, suffix="events"
        )
        bpath = bpath.update(
            description=self._description(),
            root=ctx.root / DERIV_DIR / "eelbrain",
            check=False,
        )
        return bpath.fpath
    ...
    def dependencies(self, ctx: Request) -> tuple[Dependency, ...]:
        source_node = raw_node_name(self.pipe.source)
        return (
            Dependency(source_node, options={"noise": False, "preload": True}),
        )
    ...
    def build(self, ctx: Request) -> list[tuple[str, float, float]]:
        ...
        return [
            (ch, t0 + offset, t1 + offset) for ch, t0, t1 in local_intervals
        ]

    def load(self, ctx: Request, path: Path) -> list[tuple[str, float, float]]:
        df = pd.read_csv(path, sep="\t")
        return list(
            zip(df["channel"], df["onset"], df["onset"] + df["duration"])
        )

    def save(
        self, ctx: Request, path: Path, value: list[tuple[str, float, float]]
    ) -> None:
        df = pd.DataFrame(
            {
        ...
        json_path = (
            ...
            / DERIV_DIR
            / "eelbrain"
            / f"desc-{self._description()}_events.json"
        )
        if not json_path.exists():
            json_path.write_text(
                json.dumps(_CLEAN_WINDOWS_EVENTS_JSON, indent=4)
            )
...
class CanonicalHeadPositionDerivative(Derivative):
    ...
    def build(self, ctx: Request) -> mne.transforms.Transform | None:
        ...
        for label in ctx.declared_dependencies:
            positions = ctx.load(
                label
            )  # (n, 6): [q1, q2, q3, tx, ty, tz], or None
            if positions is not None:
                all_positions.append(positions)
        if len(all_positions) <= 1:
            return None
        all_pos = numpy.vstack(
            all_positions
        )  # (N, 6): [q1, q2, q3, tx, ty, tz]
        ...
        trans = numpy.eye(4)
        trans[:3, :3] = (
            Rotation.from_quat(numpy.column_stack([q, q0])).mean().as_matrix()
        )
        trans[:3, 3] = numpy.mean(all_pos[:, 3:], axis=0)
        return mne.transforms.Transform(fro="meg", to="head", trans=trans)

    def save(
        self, ctx: Request, path: Path, value: mne.transforms.Transform | None
    ) -> None:
        if value is None:
            path.touch()
        else:
